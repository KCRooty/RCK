...
- **Fase 3** (en progreso):
  ...
  - ✅ Empaquetado real (`tauri build`): genera `RCK_0.1.0_x64-setup.exe` (NSIS) y `RCK_0.1.0_x64_en-US.msi`. Se corrigió `tauri.conf.json` para bundlear solo `catalog/{tweaks,apps,tools}`, `public_key.pem` y `catalog.sig` — el glob inicial (`../catalog/**/*`) empaquetaba por error `private_key.pem` dentro del instalador.
  - 🚧 Pendiente: navegador de apps *instaladas* con desinstalación (tipo Wintoys — distinto del catálogo curado de apps instalables), medidores en vivo de CPU/RAM/red en el dashboard, icono propio (los actuales son placeholders), y seguir ampliando cobertura de mapeo WinScript (queda un resto de claves de terceros — telemetría de Adobe/NVIDIA/VSCode, Windows Update pause, etc. — sin tweak equivalente todavía).
